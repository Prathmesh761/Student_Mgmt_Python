...
def delete():
    global TABLE_NAME
    last_entered_id = connection.execute("SELECT MAX(" + STUDENT_ID + ") FROM " + TABLE_NAME).fetchone()[0]

    if last_entered_id is not None:
        connection.execute("DELETE FROM " + TABLE_NAME + " WHERE " + STUDENT_ID + " = ?;", (last_entered_id,))
        connection.commit()
        messagebox.showinfo("Success", "Last Entered Record Deleted Successfully.")
    else:
        messagebox.showinfo("Error", "No records to delete.")
...
deletebtn = tk.Button(root, text="Delete Record", command=delete)
deletebtn.grid(row=6, column=0)

updatebtn = tk.Button(root, text="Update Record", command=update)
updatebtn.grid(row=6, column=1)
